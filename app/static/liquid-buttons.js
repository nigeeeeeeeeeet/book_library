(()=>{var k=Object.defineProperty;var W=(i,e,t)=>e in i?k(i,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):i[e]=t;var r=(i,e,t)=>W(i,typeof e!="symbol"?e+"":e,t);var P=`#version 300 es
...
}`;function N(){let i=navigator.userAgent.toLowerCase();return i.includes("safari")&&!i.includes("chrome")&&!i.includes("android")}function X(i){let e=visualViewport?.scale??1,t=visualViewport?.width??window.innerWidth,o=window.innerWidth-i.documentElement.clientWidth,n=e*t+o,s=outerWidth/n,a=Math.round(100*s);return a%5===0?a/100:a===33?1/3:a===67?2/3:a===133?4/3:s}var D=`
...
`;var q=142,j=46,U=.6,T=1,Y=2.4,y=[[0,14,11,10],[.3,88,30,22],[.52,209,87,58],[.72,242,106,75],[.88,255,196,172],[1,248,245,239]],V=(()=>{let i=new Uint8ClampedArray(768);for(let e=0;e<256;e++){let t=e/255,o=0;for(;o<y.length-2&&t>y[o+1][0];)o++;let[n,s,a,l]=y[o],[u,d,c,m]=y[o+1],f=(t-n)/(u-n);i[e*3]=s+(d-s)*f,i[e*3+1]=a+(c-a)*f,i[e*3+2]=l+(m-l)*f}return i})(),S=window.matchMedia("(prefers-reduced-motion: reduce)"),w=null,p=null,h=null,E=null,z=!1,b=null,v=0,F=new Map,g=new Set;function Z(){if(w||z)return!z;try{let i=document.createElement("div");return i.setAttribute("aria-hidden","true"),i.style.cssText=`position:fixed;left:0;top:0;width:${q}px;height:${j}px;opacity:0;pointer-events:none;z-index:-1;overflow:hidden;`,document.body.appendChild(i),w=new x(i,B,{u_repetition:4,u_softness:.5,u_shiftRed:.3,u_shiftBlue:.3,u_distortion:0,u_contour:0,u_angle:45,u_scale:8,u_shape:1,u_offsetX:.1,u_offsetY:-.1},{preserveDrawingBuffer:!0},S.matches?0:U,0,3),p=i.querySelector("canvas"),h=document.createElement("canvas"),E=h.getContext("2d",{willReadFrequently:!0}),!0}catch(i){return z=!0,document.documentElement.classList.add("lm-static"),console.warn("Liquid buttons: WebGL unavailable, using CSS fallback.",i),!1}}function _(i){w&&!S.matches&&w.setSpeed(i)}function K(){if(!p||!p.width)return!1;(h.width!==p.width||h.height!==p.height)&&(h.width=p.width,h.height=p.height),E.drawImage(p,0,0);let i=E.getImageData(0,0,h.width,h.height),e=i.data;for(let t=0;t<e.length;t+=4){let o=e[t],n=e[t+1],s=e[t+2],a=.299*o+.587*n+.114*s+.35*(o-s);a=a<0?0:a>255?255:a|0,e[t]=V[a*3],e[t+1]=V[a*3+1],e[t+2]=V[a*3+2],e[t+3]=255}return E.putImageData(i,0,0),!0}function Q(i,e){let{canvas:t,ctx:o,offset:n}=e,s=i.getBoundingClientRect();if(!s.width||!s.height||!h||!h.width)return;let a=Math.min(window.devicePixelRatio||1,2),l=Math.max(1,Math.round(s.width*a)),u=Math.max(1,Math.round(s.height*a));(t.width!==l||t.height!==u)&&(t.width=l,t.height=u);let d=h.height,c=Math.min(h.width,d*(s.width/s.height)),m=(h.width-c)*n;o.drawImage(h,m,0,c,d,0,0,l,u)}function M(){if(b=null,!K()){g.size&&(b=requestAnimationFrame(M));return}g.forEach(i=>Q(i,F.get(i))),g.size&&!S.matches&&(b=requestAnimationFrame(M))}function R(){b===null&&g.size&&(b=requestAnimationFrame(M))}function J(i){let e=document.createElement("canvas");e.className="lm-canvas",e.setAttribute("aria-hidden","true"),i.prepend(e),i.classList.add("has-canvas"),F.set(i,{canvas:e,ctx:e.getContext("2d"),offset:Math.random()})}function G(){let i=document.querySelectorAll(".btn-liquid");if(!i.length)return;let e=new IntersectionObserver(t=>{t.forEach(({target:o,isIntersecting:n})=>{if(n){if(!Z())return;F.has(o)||J(o),g.add(o)}else g.delete(o)}),w&&_(g.size?v?T:U:0),R(),S.matches&&(setTimeout(R,150),setTimeout(R,600))},{rootMargin:"80px"});i.forEach(t=>e.observe(t)),document.addEventListener("pointerover",t=>{let o=t.target.closest?.(".btn-liquid");!o||o.contains(t.relatedTarget)||(v++,_(T))}),document.addEventListener("pointerout",t=>{let o=t.target.closest?.(".btn-liquid");!o||o.contains(t.relatedTarget)||(v=Math.max(0,v-1),v||_(U))}),document.addEventListener("click",t=>{let o=t.target.closest?.(".btn-liquid");if(!o||(_(Y),setTimeout(()=>_(v?T:U),300),S.matches))return;let n=o.getBoundingClientRect(),s=document.createElement("span");s.className="ripple",s.style.left=`${t.clientX-n.left}px`,s.style.top=`${t.clientY-n.top}px`,o.appendChild(s),s.addEventListener("animationend",()=>s.remove())}),window.addEventListener("resize",R)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",G):G();})();
